...
print(train1_data.columns.values)
print(train2_data.columns.values)
X_train1 = train1_data.drop(['isDefault', 'earlies_credit_mon', 'loan_id', 'user_id', 'earlies_credit_mon'], axis=1, inplace=False)
y_train1 = train1_data['isDefault']

X_train2 = train2_data.drop(['isDefault', 'earlies_credit_mon', 'loan_id', 'user_id','issue_date', 'earlies_credit_mon'], axis=1, inplace=False)
y_train2 = train2_data['isDefault']
###   使用X_train1 用作训练集 ，X_train2 用于测试集

from sklearn.ensemble import  RandomForestClassifier
rf=RandomForestClassifier(max_features='auto',oob_score=True,random_state=1,n_jobs=-1)
##怎么处理x_train 中值为nan的数据
rf.fit(X=X_train1,y=y_train1)
pred=rf.predict(X_train2)
print(roc_auc_score(y_train2,pred))